...
const io = new Server(server, {
  ...
  // Detect a dropped driver (internet off / app killed) fast, so the rider
  // map flips to "signal lost" within a few seconds instead of ~30s.
  pingInterval: 4000,
  pingTimeout: 5000,
});
...
io.on("connection", (socket) => {
  ...
  // ---- driver logs out / leaves a vehicle: remove it entirely so it can't linger ----
  socket.on("leaveVehicle", (rec) => {
    const id = (rec && rec.id) || ownerOf[socket.id];
    if (id && vehicles[id]) delete vehicles[id];
    if (ownerOf[socket.id]) delete ownerOf[socket.id];
    if (id) io.emit("vehicleGone", { id });
  });
